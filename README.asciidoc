...
1) what do you could know after reading that:
...
Voice Request URL --> the address where your app should be avalaible, for example:
	http://test.twiliofaces.org/sample/twiml/first.twiml  (yes the extension is .twiml --> twiliofaces in the web fragment declare a Faces Servlet to parse .tmiml files like jsf files, and enable a filter to trace the xml produced and you can receive in cdi events. you will see in the follows like @observes that events).
	Twilio retrieve and execute the TwiML via the selected HTTP method when this number receives a phone call (You can receive the twilio parameters on POST or GET - in jsf is the same!).
	This is the entry point in your sample twilio app. 

Status Callback URL -->  the address where your app expose an hidden link content in twiliofaces (web-fragment): http://test.twiliofaces.org/sample/notification.jsf
	Twilio make a request to this URL when a call to this phone number is completed.
...
In the follows a minimal pom.xml
...
a beans.xml (to start CDI container):
...
== some cdi controller to play with our quiz

A simple LogController to to @observes the xml producted when twilio forward the request on our app:
...
In this controller we also want trace all parameters that twilio send tou our app. In Twiliofaces we can inject a TwilioRequestMap, for example we will see:
AccountSid, ApiVersion, ApplicationSid, CallSid, CallStatus, Caller, Direction, From, PhoneNumber 
